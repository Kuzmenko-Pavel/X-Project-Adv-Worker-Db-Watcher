...
class CampaignByDevices(ParentBase):
    ...
    id_device = Column(ForeignKey('devices.id', ondelete='CASCADE'), primary_key=True, nullable=False)

    campaign = relationship('Campaign', foreign_keys=[id_campaign], uselist=False, passive_deletes=True)
    device = relationship('Device', foreign_keys=[id_device], uselist=False, passive_deletes=True)
    ...
    def __hash__(self):
        return hash((self.id_campaign, self.id_device, self.campaign, self.device))

    def __eq__(self, other):
        return all([getattr(self, x) == getattr(other, x) for x in
                    ['id_campaign', 'id_device', 'campaign', 'device']])
